...
const ConfirmationDialog: React.FC<ConfirmationDialogProps> = ({ isOpen, onConfirm, onCancel, title, message }) => {
  ...
  return (
    <>
      ...
      <div
        className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] max-w-[90vw] max-h-[80vh] bg-[var(--bg-panel)] text-[var(--text-primary)] z-[60] rounded-3xl shadow-2xl border border-[var(--border-primary)] transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] animate-bounce-in-up flex flex-col"
        role="alertdialog"
        aria-modal="true"
