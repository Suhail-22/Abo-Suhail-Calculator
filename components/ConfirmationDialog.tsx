...
const ConfirmationDialog: React.FC<ConfirmationDialogProps> = ({ isOpen, onConfirm, onCancel, title, message }) => {
  ...
  return (
    <>
      ...
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] max-w-[90vw] bg-[var(--bg-panel)] text-[var(--text-primary)] z-[60] p-6 rounded-3xl shadow-2xl border border-[var(--border-primary)] transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] animate-bounce-in-up"
        role="alertdialog"
        aria-modal="true"
